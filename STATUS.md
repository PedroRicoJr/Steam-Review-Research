...
| In-session timer | CronCreate job `9a2cd22e`, `*/20 * * * *` (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 410 (backlog rows 261-280) |
| Next unit | Round 411: the next 20 `open` rows of `tag-tree-backlog.tsv` |
| Backlog | 192 open, 24 check |
| Tree | 1,291 tags |
