...
| In-session timer | CronCreate job `c958bd4b`, `*/20 * * * *` (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 403 (backlog rows 121-140) |
| Next unit | Round 404: the next 20 `open` rows of `tag-tree-backlog.tsv` |
| Backlog | 332 open, 24 check |
| Tree | 1,208 tags |
