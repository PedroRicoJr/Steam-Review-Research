...
| In-session timer | CronCreate job `8c3f8f4b`, `7,37 * * * *` (re-created 17:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 467: Warframe batch 60, 50 reviews, 0 modes (3,050 of 3,235) |
| Next unit | Warframe batch 61: the next **50** (`python summarise.py next --group warframe/english --n 50`) - 50 kept after the 25-review test (round 432) |
| Backlog | finished: built 347, existing 133, skip 63 (the skips wait on Rico or are jokes) |
| Tree | 1,540 tags |
| Warframe | 3,050 of 3,235 read; 185 left; next is batch 61 (50) |
