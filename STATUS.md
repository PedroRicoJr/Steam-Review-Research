...
| In-session timer | CronCreate job `a3a89019`, `7,37 * * * *` (re-created 15:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 465: Warframe batch 58, 50 reviews, 0 modes (2,950 of 3,235) |
| Next unit | Warframe batch 59: the next **50** (`python summarise.py next --group warframe/english --n 50`) - 50 kept after the 25-review test (round 432) |
| Backlog | finished: built 347, existing 133, skip 63 (the skips wait on Rico or are jokes) |
| Tree | 1,539 tags |
| Warframe | 2,950 of 3,235 read; 285 left; next is batch 59 (50) |
