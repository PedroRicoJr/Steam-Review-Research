...
| In-session timer | CronCreate job `c4290f81`, `7,37 * * * *` (re-created 19:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 495: Escape from Duckov batch 20, 50 reviews, 5 modes (1,000 of 1,166) |
| Next unit | Escape from Duckov batch 21: the next **50** (`python summarise.py next --group escape-from-duckov/english --n 50`) |
...
| Escape from Duckov | pulled 2026-09-26: 1,166 reviews (+/-2.83%); 1,000 read; next is batch 21 (50) |
