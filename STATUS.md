...
| In-session timer | CronCreate job `59d3805d`, `*/20 * * * *` (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 416 (backlog rows 381-400) |
| Next unit | Round 417: the next 20 `open` rows of `tag-tree-backlog.tsv` |
| Backlog | 72 open, 24 check |
| Tree | 1,376 tags |
