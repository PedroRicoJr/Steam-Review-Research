...
| In-session timer | CronCreate job `2a4b8dc1`, `7,37 * * * *` (re-created 14:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 490: Escape from Duckov batch 15, 50 reviews (1 excluded), 3 modes (750 of 1,166) |
| Next unit | Escape from Duckov batch 16: the next **50** (`python summarise.py next --group escape-from-duckov/english --n 50`) |
...
| Escape from Duckov | pulled 2026-09-26: 1,166 reviews (+/-2.83%); 750 read; next is batch 16 (50) |
