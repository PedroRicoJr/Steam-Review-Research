...
| In-session timer | CronCreate job `f40f27de`, `*/20 * * * *` (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 417 (backlog rows 401-420) |
| Next unit | Round 418: the next 20 `open` rows of `tag-tree-backlog.tsv` |
| Backlog | 52 open, 24 check |
| Tree | 1,387 tags |
