...
| In-session timer | CronCreate job `ab894d8c`, `7,37 * * * *` (re-created 12:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 488: Escape from Duckov batch 13, 50 reviews, 0 modes (650 of 1,166) |
| Next unit | Escape from Duckov batch 14: the next **50** (`python summarise.py next --group escape-from-duckov/english --n 50`) |
...
| Escape from Duckov | pulled 2026-09-26: 1,166 reviews (+/-2.83%); 650 read; next is batch 14 (50) |
