...
| In-session timer | CronCreate job `ec5e98e8`, `7,37 * * * *` (re-created 16:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 492: Escape from Duckov batch 17, 50 reviews, 4 modes (850 of 1,166) |
| Next unit | Escape from Duckov batch 18: the next **50** (`python summarise.py next --group escape-from-duckov/english --n 50`) |
...
| Escape from Duckov | pulled 2026-09-26: 1,166 reviews (+/-2.83%); 850 read; next is batch 18 (50) |
