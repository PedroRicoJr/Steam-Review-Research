...
| Cadence | every 5 minutes until **05:00 Pacific, 2026-09-25 (12:00 UTC)**, then **every 30 minutes** (Rico, 2026-09-25). A timer re-created after 12:00 UTC uses `7,37 * * * *`. |
| In-session timer | CronCreate job `9c9996ce`, `*/5 * * * *` until 12:00 UTC; a `send_later` check-in at 12:00 UTC (`trig_01Gn4CabtvWHf8XuvBVUwR3n`) swaps it for `7,37 * * * *` (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
