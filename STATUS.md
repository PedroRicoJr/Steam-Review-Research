...
| In-session timer | CronCreate job `798938a6`, `7,37 * * * *` (re-created 08:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 484: Escape from Duckov batch 9, 50 reviews, 1 mode (450 of 1,166) |
| Next unit | Escape from Duckov batch 10: the next **50** (`python summarise.py next --group escape-from-duckov/english --n 50`) |
...
| Escape from Duckov | pulled 2026-09-26: 1,166 reviews (+/-2.83%); 450 read; next is batch 10 (50) |
