...
| In-session timer | CronCreate job `20db51f0`, `7,37 * * * *` (re-created 02:45 UTC; the session has restarted each hour, so in practice the hourly backstop sets the pace) (auto-expires after 7 days; the backstop re-creates it) (session-only; dies when the cloud session goes idle - see OPEN-WITH-RICO.md) |
...
| Last unit done | Round 478: Escape from Duckov batch 3, 50 reviews, 3 modes (150 of 1,166) |
| Next unit | Escape from Duckov batch 4: the next **50** (`python summarise.py next --group escape-from-duckov/english --n 50`) |
...
| Escape from Duckov | pulled 2026-09-26: 1,166 reviews (+/-2.83%); 150 read; next is batch 4 (50) |
